...
const LoginActivity = () => {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0d1426]/70 p-4 sm:p-6 backdrop-blur-md space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
          onClick={handleClear}
          className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 text-xs sm:text-sm font-medium transition-all"
        >
          <Trash2 size={16} />
          Clear Activity
        </button>
      ...
      <form onSubmit={handleSearch} className="grid grid-cols-1 sm:grid-cols-12 gap-3">
        <div className="sm:col-span-6 relative">
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition"
            value={search}
        ...
        <div className="sm:col-span-3">
          <select
            className="w-full rounded-xl border border-white/10 bg-[#0d1426] px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-indigo-500 transition"
            value={action}
        ...
        <div className="sm:col-span-3">
          <button
            type="submit"
            className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-semibold text-white flex items-center justify-center gap-2 transition"
          >
            <Search size={16} />
            Filter
          </button>
          ...
          currentActivities.map((item) => (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                >
                  {item.action === "login" ? <LogIn size={11} /> : <LogOut size={11} />}
                  <span className="capitalize">{item.action}</span>
                </span>
              </div>

              <div className="grid grid-cols-2 gap-2 text-[11px] pt-1 text-gray-400 border-t border-white/5">
                <div>
                  <span className="block text-[10px] text-gray-500">IP / Provider</span>
                  {item.ipAddress || "—"} • {item.loginProvider || "pwd"}
                </div>
                <div>
                  <span className="block text-[10px] text-gray-500">Login Time</span>
                  {item.loginTime ? new Date(item.loginTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "—"}
                </div>
              </div>
            </div>
          ))
